...
| Токены НЕ передаются фронтенду | В архитектуре (на диаграмме) это достигается через BFF: access/refresh хранятся на бэкенде, фронтенд получает только сессионную cookie. **В демо-коде спринта PKCE показан на SPA-клиенте**, чтобы продемонстрировать замену Code Grant → PKCE. |
